...
| `phantombot mcp help|add|list|status|search|describe|call|login|gmail-auth|calendar-auth|remove|proxy` | Register and lazily use persona-scoped MCP servers; the Google auth commands store refreshable credentials directly in the encrypted vault |
