...
PhantomBot remains the owner of conversation history and durable memory. Its
canonical prompt architecture keeps persona instructions and security policy
in the stable system prompt, then places the current turn's retrieved context,
durable facts, daily recall, and channel metadata after canonical history and
before the current message. Volatile context follows conversation history so a
downstream prompt/KV cache can reuse the common prefix. Any llama.cpp or Pi
prompt cache is disposable acceleration; losing it never loses memory.
